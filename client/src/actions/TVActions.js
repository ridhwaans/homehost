      ...
      .then(items => {
        ...
        for (var i = 0; i < items.length; i++) {
          for (var j = 0; j < items[i].seasons.length; j++) {
            items[i].seasons[j].tmdb_id = items[i].seasons[j].id
            delete items[i].seasons[j].id
            items[i].seasons[j].poster_path = 'https://image.tmdb.org/t/p/w500' + items[i].seasons[j].poster_path

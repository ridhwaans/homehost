...
export const getImdbId = (file_path: string): string => {
  ...
  let [name_with_id, ,] = fullName.split('.');
  const splited_name_with_id = name_with_id.split(' ');
  const imdbID = splited_name_with_id[splited_name_with_id.length - 1];
  return imdbID;
};

export const shuffleArr = (arr: unknown[]) => {
...
const allowedExtensions = /\.(mp4|mkv|mp3|wav)$/i;
